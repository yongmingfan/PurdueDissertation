...
\subsection{\zksnark}
In a zero-knowledge protocol \cite{goldreich2019proofs}, a user (the prover) proves a statement to another party (the verifier) without revealing anything about the statement other than that it is true.
zkSNARKs (Zero-Knowledge Succinct Non-Interactive Arguments of Knowledge)~\cite{ben2013snarks,parno2016pinocchio,groth2016size} are one of the current most popular zero-knowledge proof systems, and have begun to see increasingly complex, real world deployment in a number of privacy-preserving applications, such as anonymous transactions, verifiable computation, and privacy-preserving identity systems.~\cite{zcash,darkforest,aleo,filecoin,zkrollup,fan2025byzsfl,kosba2016hawk,bowe2020zexe}.
At a very high level, \zksnark works as follows.  The first step in proof creation is to turn the statement that one wishes to prove into an equivalent form that relies on knowing a solution to some algebraic equations.  This representation is then broken down into an arithmetic circuit.  To ensure the correct evaluation of the circuit (and thus the proof), a programmer must express a series of constraints on the wires, called a constraint system, which is typically a Rank 1 constraint system or R1CS. 
...
Special-purpose fuzzing has been used for TLS \cite{somorovsky2016systematic,walz2017exploiting}. CDF \cite{aumasson2017automated} targets cryptographic algorithms such as DSA, ECDSA, and RSA, and works primarily by using differential fuzzing and known test vectors, which feeds in edge cases and various values, and then compares the output to either a set of known test vectors or the output of another implementation.
...
We provide a comparison of \snarkprobe and the most relevant existing fuzzing tools in \autoref{tab:snarkprobe-fuzzer_comparison}, focusing on the types of errors they are able to catch, if they can precisely locate such errors, and their adaptability to \zksnark{s}.  Currently, most fuzzing tools are limited to identifying software errors. While some tools such as CDF and SHA-3 Test have the capability to identify \logicerror{s}, their functionality is restricted and cannot be easily extended.
...
\begin{table}[t]
\caption{Comparison of \snarkprobe and a selection of cryptographic and generic fuzzing tools. A \LEFTcircle~ means achieves in select instances.}
\vspace*{6pt}
\centering
...
\textbf{\snarkprobe} & \CIRCLE & \multicolumn{1}{c|}{\CIRCLE} & \multicolumn{1}{c|}{\CIRCLE} & \CIRCLE & \multicolumn{1}{c|}{\CIRCLE} & \CIRCLE \\ \hline
\end{tabular}
\end{adjustbox}
...
Based on the prior work and current techquies, there are following challenges to evaluate the security of a \zksnark library, which are also our motivation to develop our \snarkprobe.
\begin{itemize}[leftmargin=*]
    ...
    \setlength{\parskip}{0pt}
	  ...
	  %\item \fanyo{We evaluate \snarkprobe's ability with recent \zksnark applications in academic paper and projects to find potential inconsistencies and different types of errors in zkSNARK protocols and implementations and discuss the various inconsistencies it detected.}
\end{itemize}
