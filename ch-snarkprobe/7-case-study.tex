...
\centering
\footnotesize
\caption{Summary of the current potential errors and inconsistencies, and previous vulnerabilities, that \snarkprobe is able to catch and locate.}
\vspace*{6pt}
...
The \texttt{comparison\_gadget} in \libsnark uses bit shifting to compare two variables $A$ and $B$ by calculating $2^n + B - A$ and then counting the number of 0s and 1s in the resulting bit array. The corresponding R1CS constraints are shown below, and $w_n$ represents the $n^{\text{th}}$ witness in the R1CS.


\vspace{5pt}
\begin{equation}
\label{eq:comparison_gadget}
\begin{cases}
(w_{14} * (1 + (p - 1) * w_{14})) \bmod p = 0 \\
\begin{aligned}
&(1024 + w_{3} + 2 * w_{4} + 4 * w_{5} + 8 * w_{6} + 16 * w_{7} + 32 * w_{8} \\
&\quad\quad + 64 * w_{9} + 128 * w_{10} + 256 * w_{11} + 512 * w_{12}) \bmod p = w_{13}
\end{aligned} \\
...
\begin{aligned}
&(w_{15} * (w_{3} + w_{4} + w_{5} + w_{6} + w_{7} + w_{8} + w_{9} + w_{10} + w_{11} \\
&\quad\quad + w_{12})) \bmod p = w_{14}
\end{aligned} \\
\begin{aligned}
&((1 + (p - 1) * w_{14}) * (w_{3} + w_{4} + w_{5} + w_{6} + w_{7} + w_{8} \\
&\quad\quad + w_{9} + w_{10} + w_{11} + w_{12})) \bmod p = 0
\end{aligned} \\
w_{14} \bmod p = 1
\end{cases}
\end{equation}
...
%However, one cannot guarantee that such a case will never happen in practice, which is why tools like \snarkprobe can be valuable for stress testing implementations.
...
\parhead{CVE-2019-7167 Problem.} Finally, we discuss how \snarkprobe (specifically \SnarkFuzzer) is able to detect real world vulnerabilities by demonstrating its ability to pinpoint a previous CVE.
