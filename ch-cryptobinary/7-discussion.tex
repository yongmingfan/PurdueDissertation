...
Based on our evaluation (see Table \ref{tab:cryptobinary-benchmarks}), we found that PEiD KANAL, SignSrch, and Where's Crypto can detect most algorithms we selected. This was somewhat expected as Where's Crypto is a more modern detection tool. HCD demonstrated average performance in our evaluation results.
...
\begin{figure}[t]
    \centering
  \includegraphics[width=0.9\linewidth,keepaspectratio]{ch-cryptobinary/figure/algoeasy.png}
	\vspace*{-8pt}
...
Our evaluations in this space are discussed in Sections \ref{subsec:cryptobinary-algo-version} and \ref{subsec:cryptobinary-algo-version1}.
...
Cryptographic features (see Section \ref{sec:cryptobinary-features}) present in one algorithm may not be present in others.
