...
\addbibresource{all-biblatex-greybox.bib}
...
\newcommand{\parhead}[1]{\noindent\hspace*{\parindent}{\bfseries\boldmath\ignorespaces #1}\hskip 0.9em plus 0.3em minus 0.3em}
\newcommand{\parheadit}[1]{\noindent\hspace*{\parindent}{\itshape\ignorespaces #1}\hskip 0.9em plus 0.3em minus 0.3em}
...
\newcommand*{\drawEva}[3]{%
    ...
    \hspace{-0.2em} % Space between shapes
...
\newtcolorbox{takeaway}[1][]
{
    ...
    boxsep=2pt,
    arc=0pt,
    before=\vspace{10pt}\par\noindent,
		after=\vspace{0pt},
    #1,
}

% macros
\def\snarkprobe{\texttt{SNARKProbe}\xspace}
...
\def\rapidsnark{\texttt{rapidsnark}\xspace}
\def\snarkjs{\texttt{snarkjs}\xspace}
\def\gnark{\texttt{gnark}\xspace}
\def\playsnark{\texttt{playsnark}\xspace}
\def\circom{\texttt{Circom}\xspace}
\def\groth16{Groth16\xspace}
\def\pinocchio{Pinocchio\xspace}

% ----- tmp for greybox start -----
\def\zkp{zero knowldge proof\xspace}
\def\greyboxfuzz{\texttt{GreyBox-Fuzzing}\xspace}
\def\pp{proving program\xspace}
\def\vp{verification program\xspace}
\def\programbranch{general programming branch\xspace}
\def\cryptobranch{cryptographic related branch\xspace}
\def\userp{\texttt{user}\textsubscript{\texttt{P}}\xspace}
\def\userv{\texttt{user}\textsubscript{\texttt{V}}\xspace}
\def\witnessorg{\texttt{witness}\textsubscript{\texttt{org}}\xspace}
\def\witnessint{\texttt{witness}\textsubscript{\texttt{int}}\xspace}
\def\pfunc{\ensuremath{\mathcal{P}}\xspace}
\def\vfunc{\ensuremath{\mathcal{V}}\xspace}
\def\pprog{\ensuremath{\mathcal{P}_\texttt{func}}\xspace}
\def\vprog{\ensuremath{\mathcal{V}_\texttt{func}}\xspace}
\def\inputconvert{input converter\xspace}
\def\afl{\texttt{AFL/AFL++}\xspace}
\def\diffafl{\texttt{Diff AFL}\xspace}
% ----- tmp for greybox end -----
...
\include{ch-cryptobinary}
\include{ch-snarkprobe}
\include{ch-greybox}
\include{ch-summary}
