...
\label{subsec:greybox-zkp_code_branch}
...
\label{subsec:greybox-branch_identify}

Before fuzzing begins, \greyboxfuzz identifies \cryptobranch in the proving function \pprog and verification function \vprog. In a \zksnark program, the \pprog computes proof elements from the witness, circuit, proving key, and protocol-specific randomness. The \vprog checks the proof using the public witness, verification key, and proof object. These computations depend on inputs of \zksnark program to determine whether the final verification result is satisfiable. Therefore, \greyboxfuzz uses taint tracking to identify instructions whose values are derived from, or controlled by, these inputs of \zksnark program.
...
To address this limitation, \greyboxfuzz uses a data-dependency and control-dependency-aware taint tracking strategy. To maximize the detection of \cryptobranch, we developed a new control-dependency-aware taint-tracking tool specifically designed for \zksnark programs to capture a broad range of control-dependency scenarios. 
In addition to propagating taint through explicit data movement and arithmetic operations, \greyboxfuzz also propagates taint to values assigned inside a conditional region controlled by a tainted condition. This is important for cryptographic programs because input-dependent branches may select different constants, table entries, validation outcomes, or protocol states. Even when a value is not directly computed from a tainted input, it may still influence the cryptographic result.

The static taint tracking operates on assembly instructions and maintains two taint states: tainted memory locations and tainted registers. It applies the following rules:
...
Although the taint tracking is designed for \zksnark programs in this paper, the same strategy can be applied more broadly to other cryptographic protocols whose security-relevant computation depends on identifiable protocol inputs. For example, proof systems such as Bulletproofs or other zero-knowledge protocols also compute protocol outputs from witnesses, public statements, commitments, and public parameters. As long as these inputs can be identified as taint sources, the same taint tracking strategy can be used to distinguish cryptographic computation from general program logic. 

Traditional taint tracking mainly follows explicit data dependencies, which can miss branches and values that are only influenced through control flow, such as many actual implementations in \zksnark programs. In contrast, the taint tracking in \greyboxfuzz combines data-dependency tracking with control-dependency-aware propagation, allowing it to capture branches that are directly or indirectly affected by \zksnark inputs. This makes the coverage monitor more suitable for investigating the proof generation and proof verification paths that are most relevant to cryptographic logic errors.

\subsection{Branch Tracking}
\label{subsec:greybox-branch_tracking}
...
The branch tracking result also provides a stopping criterion. Instead of requiring complete coverage of the entire binary, \greyboxfuzz can report the coverage achieved over the \cryptobranch. This allows developers to determine whether the security-relevant portion of the target program has been sufficiently tested.