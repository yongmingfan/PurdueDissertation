...
\label{sec:greybox-input_formatter}

The effectiveness of fuzzing depends heavily on the quality of the generated inputs. For programs with simple input formats, random mutation may be sufficient to reach meaningful program states. However, \zksnark programs require highly structured inputs, including public witnesses, private witnesses, finite-field elements, elliptic-curve points, keys, proofs, and circuit-related data. If these inputs do not satisfy the expected format, the program often rejects them during parsing or early validation. As a result, a general-purpose fuzzer may spend most of its time generating inputs that never reach the cryptographic computation where \logicerror may occur.

To address this problem, \greyboxfuzz uses an input formatter that generates and mutates seeds according to the structure of the target statement. The input formatter does not attempt to ensure that every generated seed is semantically valid. Instead, its goal is to preserve the legal input format required by the target \zksnark program while still allowing the fuzzer to explore valid values, invalid values, and boundary cases. \greyboxfuzz is designed to focus on detecting \logicerror, since general-purpose fuzzing already performs well on software errors. Malformed inputs are often rejected by the \zksnark program immediately before the core protocol is executed, and therefore are less likely to expose \logicerror in proving function \pfunc or verification function \vfunc. This design allows \greyboxfuzz to avoid wasting most fuzzing iterations on malformed inputs and increases the probability that generated seeds reach the cryptographic computation.

The input formatter is built on the statement described in Section~\ref{sec:greybox-differential}. Since the user expresses the intended statement in Charm, \greyboxfuzz can use the same statement description to infer the structure of the required inputs. For example, a Charm statement may declare variables as integers, finite-field elements, group elements, or other cryptographic objects. \greyboxfuzz extracts this information and uses it to guide seed generation and mutation. This provides a lightweight structure-aware fuzzing layer without requiring source-code access to the target \zksnark binary.

\subsection{Seed Scheduling}

Not all generated seeds are equally useful for detecting \logicerror. Some inputs are rejected before they reach the cryptographic computation, while others exercise proof generation and verification but represent invalid statement-level values. To prioritize more useful seeds, \greyboxfuzz classifies generated inputs using three properties: structure, mathematical validity, and statement-level value. \autoref{tab:seed_scheduling} shows the scheduling priority used by \greyboxfuzz.
...
This scheduling policy gives the highest priority to seeds that are most likely to reach cryptographic computation and exercise statement-level behavior. The table does not enumerate every theoretical permutation of the three properties because they are not fully independent in practice. In particular, if a seed has an invalid structure, then its mathematical validity and statement-level value are usually not meaningful to evaluate. Similarly, if a seed is mathematically invalid, its statement-level value is typically treated as invalid as well. For this reason, \autoref{tab:seed_scheduling} summarizes the meaningful seed categories used by \greyboxfuzz rather than all possible combinations. Lower-priority invalid seeds are still preserved because they may expose input-validation bugs or software errors, but \greyboxfuzz assigns less fuzzing effort to structurally invalid seeds because they are unlikely to reveal \logicerror in proof generation or verification.
...
This process allows \greyboxfuzz to generate inputs that match the type expectations of both the Charm and the target \zksnark program. For example, consider a statement of the form \(h = g^x\), where \(g\) and \(h\) are public group elements and \(x\) is a private exponent. From this statement, \greyboxfuzz learns that \(g\) and \(h\) should be generated as group elements and that \(x\) should be generated as a scalar value in the appropriate field. The formatter can then generate seeds that preserve this structure while varying the actual values used for testing.
...
Grammar-based seed generation produces structured initial seeds that conform to a predefined grammar for the target program's input format. In \greyboxfuzz, the grammar is derived from the target statement and specifies the expected input variables, their types, and how they are organized into a legal input instance. The formatter first generates seeds from the extracted input structure. Each generated seed follows the expected format of the target statement. For example, if an input is a finite-field element, the formatter generates a value in the corresponding field representation. If an input is a group element, the formatter generates a value using the representation expected by the selected group. This strategy increases the probability that generated seeds pass parsing and early validation, allowing the execution to reach proof generation and verification.
...
\greyboxfuzz also generates negative test cases. These seeds intentionally violate mathematical or statement-level requirements while preserving the legal format. For example, a seed may use a value outside the intended application range, or it may fail to satisfy the statement while still being correctly encoded. Such seeds are important because a secure implementation should reject them. If the target program accepts one of these seeds, \greyboxfuzz reports a potential \logicerror.

Legal format inputs are the primary focus because they are more likely to reach the cryptographic computation.  Among them, both accepting and rejecting cases are necessary: accepting cases test whether valid witnesses are handled correctly, while rejecting cases test whether invalid witnesses are incorrectly accepted. Illegal format inputs are assigned lower priority but are still useful for detecting ordinary software errors and input validation failures.

By combining statement-derived structure, scheduling priorities assigned to different seed categories, and structure-preserving mutation, the input formatter improves the efficiency of fuzzing \zksnark programs. It reduces the number of seeds rejected before cryptographic execution and increases the number of test cases that exercise proof generation, proof verification, and statement-level correctness.
